...
3. Under users, you should only see the `github-user` user account
...
Now that the cluster and all AWS resources have been created, you'll need to add the `github-user` IAM user ARN to the Kubernetes configuration that will allow that user to execute `kubectl` commands against the cluster.
